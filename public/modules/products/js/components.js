...
function ArsenalScreen() {
    ...
    if (state.filter) {
        ...
        filtered = filtered.filter(p => p.name && p.name.toLowerCase().includes(term));
    }
            ...
            <div class="space-y-6 p-4 animate-fade-in pb-20">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex gap-2">
                        <button onclick="updateInspectorData('is_published', true); updateInspectorData('is_stock_item', true); renderModule()" class="flex-1 py-2 border ${p.is_published ? 'border-[#39FF14] bg-[#39FF14]/10 text-[#39FF14]' : 'border-zinc-800 text-zinc-600'} text-[9px] font-bold uppercase">📦 Público / Stock</button>
                        <button onclick="updateInspectorData('is_published', false); updateInspectorData('is_stock_item', false); renderModule()" class="flex-1 py-2 border ${!p.is_published ? 'border-purple-500 bg-purple-500/10 text-purple-400' : 'border-zinc-800 text-zinc-600'} text-[9px] font-bold uppercase">🎨 Privado / Custom</button>
...
window.updateCreateData = (key, val) => { 
    window.createState.data[key] = val; 
    // Synergy: Public = Stock, Private = Custom
    if (key === 'isPublished') {
        window.createState.data.isStockItem = val;
    } else if (key === 'isStockItem') {
        window.createState.data.isPublished = val;
    }
};
...
window.loadPackagingIntoSelect = async () => {
    ...
    setTimeout(() => {
        ...
        if (sel && packs.length > 0 && sel.options.length <= 1) {
            ...
            if (window.createState.data.packagingId) {
                sel.value = window.createState.data.packagingId;
                const selectedOpt = sel.options[sel.selectedIndex];
                if (selectedOpt && selectedOpt.value) {
                    window.createState.data.packagingName = selectedOpt.text;
                }
            }
        }
    }, 100);
...
window.selectQuoteForImport = (quoteId) => {
    ...
    // Embalaje: asignar y resolver nombre
    if (logistics.packagingMaterialId) {
        s.packagingId = logistics.packagingMaterialId;
        s.packagingName = "Empaque importado"; // placeholder
        if (window.fetchPackagingOptions) {
            window.fetchPackagingOptions().then(packs => {
                const match = packs.find(p => String(p.id) === String(s.packagingId));
                if (match) s.packagingName = `${match.name} (${match.sku || ''})`;
            });
        }
    }
