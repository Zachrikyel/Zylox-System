...
window.showNotification = (message, type = 'warning', duration = 3000) => {
    const existing = document.getElementById('zylox-notification');
    if (existing) existing.remove();

    const colors = {
        warning: { bg: 'bg-yellow-500/10', border: 'border-yellow-500/50', text: 'text-yellow-400', icon: '⚠️' },
        ...
        info: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/50', text: 'text-cyan-400', icon: 'ℹ️' }
    };
    const c = colors[type] || colors.warning;

    const notification = document.createElement('div');
    notification.id = 'zylox-notification';
    notification.className = `fixed top-4 left-1/2 -translate-x-1/2 z-[200] ${c.bg} ${c.border} border ${c.text} px-6 py-4 max-w-sm text-center animate-fade-in`;
    notification.style.clipPath = 'polygon(0 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%)';
    notification.innerHTML = `<span class="text-lg mr-2">${c.icon}</span><span class="text-sm font-medium">${message}</span>`;
    ...
    const listHtml = filtered.map(p => {
        ...
        <div onclick="openProductInspector('${p.id}')" class="bg-zinc-900 border border-zinc-800 h-28 flex cursor-pointer hover:border-[#39FF14] transition-all group overflow-hidden relative" style="clip-path: polygon(0 0, 100% 0, 100% 85%, 95% 100%, 0 100%);">
            ...
            <div class="flex-1 p-3 flex flex-col justify-between">
                ...
                <div>
                    <h3 class="text-sm font-bold text-white uppercase leading-tight line-clamp-2">${p.name} ${pubStatus}</h3>
...
window.deleteVariantAction = async (id) => {
    if (confirm("¿Borrar variante?")) {
        try {
...
window.saveVariantEdit = async (id) => {
    ...
    try {
        ...
        await window.updateVariantFull(id, pId, newName, newHex, imgs);
...
window.openQuoteImporter = async () => {
    ...
    if (quotes.length === 0) return showNotification("No hay cotizaciones recientes sin vincular.", "info");
    const modalHtml = `
        <div class="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
            <div class="bg-zinc-900 border border-zinc-700 w-full max-w-sm max-h-[80vh] overflow-y-auto p-4 shadow-neo">
                ...
                <button onclick="document.getElementById('importer-modal').remove()" class="w-full mt-4 py-3 border border-red-900 text-red-500 text-xs font-bold uppercase">Cancelar</button>
            </div>
        </div>
    `;
