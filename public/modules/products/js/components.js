    ...
    const modalHtml = `
        <div class="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
            <div class="bg-zinc-900 border border-zinc-700 w-full max-w-sm max-h-[80vh] overflow-y-auto p-4 shadow-neo">
                ...
                <div class="space-y-2">
                    ${quotes.map(q => `
                        <button onclick="selectQuoteForImport('${q.id}', '${q.quote_name.replace(/'/g, "")}', ${q.results.finalPrice}, ${q.results.totalProductionTime}, ${q.results.breakdown.energy}, ${q.results.breakdown.wear}, ${q.results.netProfit}, ${((q.print_data || q.print || {}).materialCost || 0) / ((q.config || {}).materialCostPerKg || 75000) * 1000}, ${q.is_free_shipping || false})" 
...
window.selectQuoteForImport = (id, name, basePrice, time, energy, wear, margin, weightGrams, isFreeShipping) => {
    const s = window.createState.data;
    ...
    s.isFreeShipping = isFreeShipping || false;
